    ...
    <div class="layui-form-item">
        ...
        <div class="layui-input-inline">
            <select name="pid" class="field-role_id" type="select">
                ...
                <option value="{$vol['id']}"
                {if condition="$vol['id'] eq $data['pid']"} selected{/if}
                >{$vol['level']}{$vol['name']}</option>
                {/volist}
            </select>
        </div>
    </div>
    <input type="hidden" name="idd" class="" value="{$data['id']}">
    <input type="hidden" name="status" class="" value="{$data['status']}">
    <div class="layui-form-item">
        <label class="layui-form-label">分类名称</label>
        <div class="layui-input-inline">
            <input type="text" class="layui-input field-username" name="name" lay-verify="title" value="{$data['name']}" autocomplete="off" placeholder="请输入分类名称">
        </div>
